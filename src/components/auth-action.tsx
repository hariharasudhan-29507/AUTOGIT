...
export function AuthAction({ mode, pending = false }: { mode: 'login' | 'signup'; pending?: boolean }) {
  ...
  if (!configured) {
    return (
      <Button disabled className="h-12 w-full justify-center">
  ...
  const child = (
    <Button
      ...
      className="h-12 w-full justify-center gap-3 bg-white text-black hover:bg-white/85 shadow-sm transition-all"
    >
  ...
  return mode === 'signup' ? (
    ...
    <SignInButton mode="modal" forceRedirectUrl="/onboarding" fallbackRedirectUrl="/onboarding">
      {child}
    </SignInButton>
  )
}
